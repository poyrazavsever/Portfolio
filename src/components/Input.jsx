...
export default function Input({label, type = 'text', ...props}) {
	...
	return (
		<label className="relative transition-all w-72 text-white flex bg-transparent border border-linear-default rounded-sm focus-within:border-primary-200">
			...
			<small className="absolute top-1/2 left-[9px] cursor-text pointer-events-none text-xs text-linear-default -translate-y-1/2 transition-all peer-valid:text-[10px] peer-valid:top-2.5">{label}</small>
